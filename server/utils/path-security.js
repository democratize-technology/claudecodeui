...
export function validateAndSanitizePath(inputPath, expectedBaseDir = null) {
  ...
  // CRITICAL SECURITY FIX: Apply Unicode normalization BEFORE pattern checking
  // This prevents Unicode normalization attacks where characters like \uFF0E\uFF0E\uFF0F
  // normalize to dangerous patterns like "../" after NFKC/NFKD normalization
  let normalizedPath;
  try {
    // Apply all normalization forms to catch any potential bypass
    const nfc = decodedPath.normalize('NFC');
    const nfd = decodedPath.normalize('NFD');
    const nfkc = decodedPath.normalize('NFKC');
    const nfkd = decodedPath.normalize('NFKD');

    // Use NFKC as the primary normalized form (most aggressive compatibility normalization)
    // but check all forms for suspicious patterns
    normalizedPath = nfkc;
    const allNormalizedForms = [nfc, nfd, nfkc, nfkd];

    // Check ALL normalized forms for suspicious patterns
    ...
    for (const normalizedForm of allNormalizedForms) {
      for (const pattern of suspiciousPatterns) {
        if (pattern.test(normalizedForm)) {
          throw new Error(
            `Invalid path: contains suspicious pattern after Unicode normalization (${pattern.source})`
          );
        }
      }
    }
  } catch (error) {
    if (error.message.includes('suspicious pattern')) {
      throw error; // Re-throw our security errors
    }
    throw new Error(`Invalid path: Unicode normalization failed (${error.message})`);
  }
  ...
  if (expectedBaseDir) {
    ...
    if (
      !resolvedPath.startsWith(canonicalBaseDir + path.sep) &&
      resolvedPath !== canonicalBaseDir
    ) {
    ...
    const isAllowed = ALLOWED_BASE_DIRECTORIES.some((baseDir) => {
      ...
      return (
        resolvedPath.startsWith(canonicalBaseDir + path.sep) || resolvedPath === canonicalBaseDir
      );
    });
