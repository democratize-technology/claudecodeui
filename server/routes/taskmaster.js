...
} from '../utils/taskmaster-websocket.js';
...
router.get('/detect/:projectName', async (req, res) => {
  try {
    ...
    if (req.app.locals.wss) {
      broadcastTaskMasterProjectUpdate(req.app.locals.wss, projectName, taskMasterResult);
    }
  ...
  try {
    ...
    try {
      ...
      const tasksResponse = await fetch(
        `${req.protocol}://${req.get('host')}/api/taskmaster/tasks/${encodeURIComponent(projectName)}`,
        {
          headers: {
  ...
  try {
    ...
    if (!fileName.match(/^[\w\-. ]+\.(txt|md)$/)) {
      return res.status(400).json({
        ...
        message:
          'Filename must end with .txt or .md and contain only alphanumeric characters, spaces, dots, and dashes'
      });
    }
