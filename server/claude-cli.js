    ...
    if (images && images.length > 0) {
      try {
        ...
        if (tempImagePaths.length > 0 && command && command.trim()) {
          ...
          if (
            printIndex !== -1 &&
            printIndex + 1 < args.length &&
            args[printIndex + 1] === command
          ) {
        ...
        if (fsSync.existsSync(claudeConfigPath)) {
          try {
            ...
            const hasGlobalServers =
              claudeConfig.mcpServers && Object.keys(claudeConfig.mcpServers).length > 0;
            const currentProjectPath = process.cwd();
            const projectConfig =
              claudeConfig.claudeProjects && claudeConfig.claudeProjects[currentProjectPath];
    ...
    claudeProcess.on('close', async (code) => {
      ...
      if (claudeProcess.tempImagePaths && claudeProcess.tempImagePaths.length > 0) {
        ...
        if (claudeProcess.tempDir) {
          await fs
            .rm(claudeProcess.tempDir, { recursive: true, force: true })
            .catch((err) =>
...
export { spawnClaude, abortClaudeSession };
