...
function printAnalysis() {
  ...
  topLargeFiles.forEach((file, index) => {
    ...
    console.log(`   ${sizeColor}Size: ${formatBytes(file.size)}${colors.reset} | Lines: ${file.lines}`);
  });
  ...
  if (buildFiles.length > 0) {
    ...
    jsFiles.forEach(file => {
      ...
      const sizeColor = file.size > 500000 ? colors.red :
                       file.size > 200000 ? colors.yellow : colors.green;
      console.log(`  ${filename}: ${sizeColor}${formatBytes(file.size)}${colors.reset} (estimated gzipped: ${formatBytes(file.estimatedGzipped)})`);
    });
    ...
    console.log(`${colors.bright}Estimated Gzipped: ${formatBytes(Math.floor(totalBuildSize * 0.3))}${colors.reset}\n`);
  }
  ...
  if (recommendations.length > 0) {
    ...
    recommendations.forEach((rec, index) => {
      const severityColor = rec.severity === 'high' ? colors.red :
                           rec.severity === 'medium' ? colors.yellow : colors.green;
  ...
  const scoreColor = performanceScore > 80 ? colors.green :
                    performanceScore > 60 ? colors.yellow : colors.red;
  
  console.log(`${colors.bright}Performance Score: ${scoreColor}${performanceScore}/100${colors.reset}`);
}
...
function calculatePerformanceScore(sourceFiles, buildFiles) {
  ...
  const totalBuildSize = buildFiles.reduce((sum, file) => sum + file.size, 0);
  if (totalBuildSize > 2 * 1024 * 1024) score -= 20; // -20 for >2MB
